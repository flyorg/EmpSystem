...
function randomimg()
{
	...
	img.src="createcode.action?"+Math.random(1,100);
}
    ...
    <section class="loginForm"> 
      ...
      <div class="loginForm_content"> 
        <fieldset> 
          ...
          %> 
            <input type="text" name="username" placeholder="邮箱/会员帐号/手机号" value="<%=username %>" autofocus required> 
          </div> 
          <div class="inputWrap"> 
            <input type="password" name="password" placeholder="请输入密码" required> 
          </div> 
          <div class="inputWrap">
          <input type="text" name="checkcode" style="width:50%" placeholder="请输入验证码" required>
          <img id="image" alt="/createcode.action" src="createcode.action" onclick="randomimg()" align="center"/>
          </div>
        </fieldset> 
        <fieldset align="right"> 
            <% if(request.getAttribute("error")!=null)
			{
				...
				if(!(error.isEmpty()))
				{
					out.println("<a href='javascript:void(0)' style='color:red'>"+error+"</a>");
				}
			}
            if(request.getAttribute("errorimg")!=null)
			{
				String errorimg=request.getAttribute("errorimg").toString();
				if(!(errorimg.isEmpty()))
				{
					out.println("<a href='javascript:void(0)' style='color:red'>"+errorimg+"</a>");
				}
			}
            if(request.getAttribute("success")!=null)
            {
				...
				if(!(success.isEmpty()))
				{
					out.println("<a href='javascript:void(0)' style='color:green'>"+success+"</a>");
					out.println("<meta http-equiv='refresh' content='1;url=success.jsp'>");
				}
            }
			%>
			<a href="javascript:void(0)" onclick="randomimg()">看不清楚换一张&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a>
        </fieldset> 
        <fieldset align="center"> 
          <input type="checkbox" checked="checked" name="savechecked" value="checked"> 
          <span>下次自动登录</span>
          <input type="submit" value="登录"> 
          <a href="find.jsp">忘记密码？</a> <a href="regest.jsp" target="_blank">注册</a> 
        </fieldset> 
      </div> 
    </section> 
  </form> 
